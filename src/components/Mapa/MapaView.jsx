...
import { COLORS } from '../../theme'; // Importamos el branding central
  ...
  return (
    <div style={{ width: '100%', height: '100%', position: 'relative', borderRadius: '24px', overflow: 'hidden' }}>
      <Map
        ...
        projection="mercator"
        // Estilo Light v11 es el más limpio para el color Linen de fondo
      ...
      >
        <Source id="world" type="vector" url="mapbox://mapbox.country-boundaries-v1">
          <Layer
            ...
            paint={{
              // Color de los países visitados (usamos Muted Teal)
              'fill-color': COLORS.mutedTeal,
              'fill-opacity': [
                'match',
                ['get', 'iso_3166_1_alpha_3'],
                paises.length > 0 ? paises : [''], 
                0.7, // Opacidad para países visitados
                0    // Invisible para los demás
              ]
            }}
          />
        </Source>
        ...
        {/* TOOLTIP DINÁMICO CON COLORES DEL TEMA */}
        {hoverInfo && (
          <div style={{ 
            position: 'absolute', 
            left: hoverInfo.x + 15, 
        ...
        {/* MODAL PERSONALIZADO CON BRANDING KEEPTRIP */}
        {modalInfo && (
            ...
            <div style={{...modalContentStyle, backgroundColor: COLORS.linen}}>
              ...
              <p style={{ color: COLORS.charcoalBlue, opacity: 0.8, fontSize: '0.95rem' }}>
                {modalInfo.yaVisitado 
                  ? `¿Eliminar ${modalInfo.name} de tu bitácora?` 
                  : `¿Marcar ${modalInfo.name} como visitado?`}
              </p>
              <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '24px' }}>
                <button onClick={() => setModalInfo(null)} style={btnSecundario}>
                  Cancelar
                </button>
                <button 
                  onClick={togglePais} 
                  style={{ 
                    ...btnPrincipal, 
                    backgroundColor: modalInfo.yaVisitado ? '#e54b4b' : COLORS.atomicTangerine 
                  }}
                >
...
const modalOverlayStyle = { 
  ...
  width: '100%', 
  height: '100%', 
  backgroundColor: 'rgba(44, 62, 80, 0.4)', // Charcoal Blue con opacidad
